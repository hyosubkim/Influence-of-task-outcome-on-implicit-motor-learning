...
%MTs and RTs for different phases
rt_bsl = nanmean(rt_cycle(:,fbBslCycles),2);
earlyclampRT = nanmean(rt_cycle(:,earlyclamp),2);
lateclamp1RT = nanmean(rt_cycle(:,lateclamp1),2);
lateclamp2RT = nanmean(rt_cycle(:,lateclamp2),2);


mt_bsl = nanmean(mt_cycle(:,fbBslCycles),2);
earlyclampMT = nanmean(mt(:, earlyclamp),2);
lateclamp1MT = nanmean(mt(:, lateclamp1),2);
lateclamp2MT = nanmean(mt(:, lateclamp2),2);
...
[h_r(i) p_r(i)] = swtest(earlyadapt(grp==grpid(i)))
[h_a(i) p_a(i)] = swtest(asymptote1(grp==grpid(i)))
end

p = vartestn(earlyadapt,grp,'TestType','LeveneAbsolute')
p = vartestn(asymptote1,grp,'TestType','LeveneAbsolute')
...
mean(bsl_var_ind(grp==3))
nanstderr(bsl_var_ind(grp==3))
...
X(:,1) = [mt_bsl; earlyclampMT; lateclamp1MT];
X(:,2) = repmat(grp',[3,1]);
X(:,3) = [ones(length(rt_bsl),1); ones(length(earlyclampRT),1)*2; ones(length(lateclamp1RT),1)*3];
...
%     
%     mean(lateclamp1RT(grp==grpid(ii)))
%     nanstderr(lateclamp1RT(grp==grpid(ii)))

%     mean(lateclamp2RT(grp==grpid(ii)))
%     nanstderr(lateclamp2RT(grp==grpid(ii)))
    ...
    mean(lateclamp1MT(grp==grpid(ii)))
    nanstderr(lateclamp1MT(grp==grpid(ii)))
    
    mean(lateclamp2MT(grp==grpid(ii)))
    nanstderr(lateclamp2MT(grp==grpid(ii)))
...
c_asy1 = multcompare(stats_asy1,'ctype','bonferroni')
...
permutation_test(asymptote1, grp, mean(asymptote1(grp==1)-asymptote1(grp==2)), 1e4)
...
% %Scheffe contrasts -- for comparing group 1 against groups 2 and 3
% MSE = tbl_asy1{3,4};
...
% end
% c=[c1;c2;c3];
% m1=mean(asymptote1(grp==1))
% sd1=std(asymptote1(grp==1))
% m2=mean(asymptote1(grp==2))
% sd2=std(asymptote1(grp==2))
% m3=mean(asymptote1(grp==3))
% sd3=std(asymptote1(grp==3))
% 
% Yhat=[m1;m2;m3];
% [n1 n2 n3]=deal(sum(grp==1));
% n=[n1;n2;n3];
% 
% Lhat = c'*Yhat
...
%
% ANALYSES FOR INITIAL SUBMISSION ONLY:
%
...
% 
% %% Quantify quality of model fitting with R^2
% 
